...
i havem issed a part of the lesson (5 min)
...
- COMMAND rs.printSecondaryReplicationInfo()  -> shows current replication lag
...
we nned to meet a RTO backup restore time and need parallelism to do so, how much time can we wait to restore from a failure?
...
Metadata cluster --> we will have one lcuster per shard, the mongoS's AND A CONFIGURATION CLUSTER with primary and secondaries
pRIMARY
secondary
secondary
