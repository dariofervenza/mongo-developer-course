...
- others:
    ...
    - wildcard: to index all the fields in the documents, each entry has all fields
...
```sh
db.a.createIndex({email: 1}, {partialFilterExpression: {status: "active"}}) --> INMDEXES email field in the documents that have the field status equal to "active"
```
